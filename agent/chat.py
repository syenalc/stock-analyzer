...
from agent.tools import TOOL_DEFINITIONS, WEB_SEARCH_TOOL, execute_tool
...
利用方針:
- **回答前に必ず最初に `get_manual_notes` を呼び、ユーザーの投資方針・業界知識・共通ナレッジを把握すること**
- 質問に答える前に、関連データを `query_tweets` や `get_price_history` でも集めること
- X APIから新規取得（fetch_user_tweets）は**ユーザーが明示的に指示した時のみ**実行。お金がかかる
- 「今〜どう思う？」「分析して」と言われた場合は、まずSupabaseの保存済みデータ（get_manual_notes, query_tweets, get_price_history）を使う
- **Web検索（web_search）**: 最新ニュース・決算速報・市場動向など、Supabaseにないリアルタイム情報が必要な時に使う。「ネットも調べて」「最新ニュース」「今日の市場」等で明示された時、または保存データだけでは答えが古すぎる時に呼ぶ。1質問あたり最大5回まで
...
def run_agent(
    ...
    tools_list = TOOL_DEFINITIONS + [WEB_SEARCH_TOOL]
    ...
    for turn in range(MAX_TOOL_TURNS):
        ...
        for block in resp.content:
            if block.type == "text":
                ...
                # server_tool_use / web_search_tool_result 等のサーバサイドブロック
                if hasattr(block, "model_dump"):
                    assistant_content.append(block.model_dump(exclude_none=True))
                    if block.type == "server_tool_use":
                        tool_trace.append(f"web_search:{getattr(block, 'name', '?')}")
